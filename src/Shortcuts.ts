...
export function register() {
    ...
    ui.registerShortcut({
        ...
        callback: () => Settings.filter.banner = !Settings.filter.banner,
    });
    ui.registerShortcut({
        id: "scenery-manager.filter.entrance",
        text: "[SM] Toggle entrance",
        bindings: ["CTRL+2", "GUI+2"],
        callback: () => Settings.filter.entrance = !Settings.filter.entrance,
    });
    ui.registerShortcut({
        id: "scenery-manager.filter.footpath",
        text: "[SM] Toggle footpath",
        bindings: ["CTRL+3", "GUI+3"],
        callback: () => Settings.filter.footpath = !Settings.filter.footpath,
    });
    ui.registerShortcut({
    ...
    ui.registerShortcut({
        ...
        callback: () => Settings.filter.large_scenery = !Settings.filter.large_scenery,
    });
    ui.registerShortcut({
        id: "scenery-manager.filter.small_scenery",
        text: "[SM] Toggle small scenery",
        bindings: ["CTRL+6", "GUI+6"],
        callback: () => Settings.filter.small_scenery = !Settings.filter.small_scenery,
    });
    ui.registerShortcut({
        id: "scenery-manager.filter.track",
        text: "[SM] Toggle track",
        bindings: ["CTRL+7", "GUI+7"],
        callback: () => Settings.filter.track = !Settings.filter.track,
    });
    ui.registerShortcut({
        id: "scenery-manager.filter.wall",
        text: "[SM] Toggle wall",
        bindings: ["CTRL+8", "GUI+8"],
        callback: () => Settings.filter.wall = !Settings.filter.wall,
    });
    ui.registerShortcut({
        id: "scenery-manager.filter.all",
        text: "[SM] Enable all",
        bindings: ["CTRL+9", "GUI+9"],
        callback: () => { for (const type in Settings.filter) Settings.filter[type] = true; },
    });
    ui.registerShortcut({
        id: "scenery-manager.filter.none",
        text: "[SM] Disable all",
        bindings: ["CTRL+0", "GUI+0"],
        callback: () => { for (const type in Settings.filter) Settings.filter[type] = false; },
    });
